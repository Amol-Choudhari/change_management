...
class DateTimeType extends BaseType implements BatchCastingInterface
{
    ...
    public function toPHP($value, DriverInterface $driver)
    {
		...
		}else{//this else part added on 10-02-2021 by Amol
		...
		//this variable added on 10-02-2021 by Amol
		$dateformat = $this->checkDateTime($value,$instance);

        if ($this->setToDateStart) {
            $instance = $instance->setTime(0, 0, 0);
			
			//these conditions added on 10-02-2021 by Amol
			if($dateformat=='dateTime'){
				
				$instance = $instance->format('d/m/Y H:i:s');
				
			}elseif($dateformat=='date'){
				
				$instance = $instance->format('d/m/Y');
				
			}elseif($dateformat=='Time'){
				
				$instance = $instance->format('H:i:s');
			}
    ...
    public function manyToPHP(array $values, array $fields, DriverInterface $driver): array
    {
        foreach ($fields as $field) {
			...
			}else{//this else part added on 12-02-2021 by Amol
				$value = str_replace('/','-',$value);
			}
            ...
            if (is_int($value)) {
                $instance = new $class('@' . $value);
            } else {//print_r($this->dbTimezone);exit;
                $instance = new $class($value, $this->dbTimezone);
            }
			...
			//this variable added on 12-02-2021 by Amol
			$dateformat = $this->checkDateTime($value);
			
            if ($this->setToDateStart) {
                $instance = $instance->setTime(0, 0, 0);
			
				//these conditions added on 12-02-2021 by Amol
				if($dateformat=='dateTime'){
	...
	//added this function on 12-02-2021 by Amol to check datetime format
	public function checkDateTime($value){
		
		$explodeTime = explode(':',$value);
		$explodeDate = explode('-',$value);
		$explodeTimeCount = count($explodeTime);
		$explodeDateCount = count($explodeDate);
		if($explodeTimeCount > 1 and $explodeDateCount > 1 ){
			return 'dateTime';
		}elseif($explodeTimeCount > 1 ){
			return 'Time';
		}elseif($explodeDateCount > 1 ){
			return 'date';
		}
