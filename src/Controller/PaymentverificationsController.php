...
class PaymentverificationsController extends AppController{
	...
	var $name = 'Paymentverifications';
				...
				////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					/*	$current_action = $this->request->getParam['action'];
						...
						if($current_action == 'home'){
							...
							$table = 'Dmi_applicant_payment_detail';
							$this->Session->write('payment_table',$table);

						}elseif($current_action == 'renewal_home'){

							$table = 'Dmi_renewal_applicant_payment_detail';
							$this->Session->write('payment_table',$table);
						}

						$table = $this->Session->read('payment_table');
						if(empty($table)){
					...
					/*	$user_name = $this->Session->read('username');
						$user_id = $this->Dmi_user->find('first', array('fields'=>'id', 'conditions' => array('email'=>$user_name)));
						$pao_id = $this->Dmi_pao_detail->find('first', array('fields'=>'id', 'conditions' => array('pao_user_id'=>$user_id['Dmi_user']['id'])));

						// Find aaplication list District wise (Updated Date : 02/05/2018 Pravin)
						$district_id_list = $this->Dmi_district->find('list', array('fields'=>'id', 'conditions' => array('pao_id'=>$pao_id['Dmi_pao_detail']['id'])));
						$check_payment_submitted_listxy = $this->$table->find('all', array('fields'=>'customer_id','conditions' => array('district_id'=>$district_id_list),'group'=>array('customer_id')));
						$list_customer_id = array();
						if(!empty($check_payment_submitted_listxy)){
							foreach($check_payment_submitted_listxy as $customer_id){

								$check_payment_submitted_listxxxxx = $this->$table->find('first', array('fields'=>array('id'),'conditions' => array('customer_id'=>$customer_id[$table]['customer_id'])));
								$list_customer_id[] = $check_payment_submitted_listxxxxx[$table]['id'];
							}
						}
						$check_payment_submitted_list = $this->$table->find('all', array('fields'=>array('customer_id'),'conditions' => array('id'=>$list_customer_id),'order'=>array('id desc')));

						//$check_payment_submitted_list = $this->$table->find('all', array('fields'=>'customer_id','conditions' => array('pao_id'=>$pao_id['Dmi_pao_detail']['id']),'group'=>array('customer_id')));
						$this->Paymentnewappcount->display_payment_verification_count($check_payment_submitted_list,$table);//calling custom function to display payment verification tab counts
						*/
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			/*
			public function pending_payment_verification(){
				...
				// Find aaplication list District wise (Updated Date : 02/05/2018 Pravin)
				$district_id_list = $this->Dmi_district->find('list', array('fields'=>'id', 'conditions' => array('pao_id'=>$pao_id['Dmi_pao_detail']['id'])));
				$check_payment_submitted_listxy = $this->$table->find('all', array('fields'=>array('customer_id'),'conditions' => array('district_id'=>$district_id_list),'group'=>array('customer_id')));
				$list_customer_id = array();
				if(!empty($check_payment_submitted_listxy)){
					foreach($check_payment_submitted_listxy as $customer_id){
				...
				//$check_payment_submitted_list = $this->$table->find('all', array('fields'=>'customer_id','conditions' => array('pao_id'=>$pao_id['Dmi_pao_detail']['id']),'group'=>array('customer_id')));
				$this->Paymentnewapplicationslist->display_payment_verification_list($check_payment_submitted_list,$table);//calling custom function to display applications list

				//to render the element view for ajax request
				$this->render('/elements/payment_details_elements/payment_applications_list_in_verified_mode');

			}

			public function payment_not_confirmed(){
			...
			public function re_submit_payment_details(){
			...
			if (!empty($customer_id)) {
				...
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					/*$this->loadComponent('Commoncountsfunctions');
								if($payment_confirmation_query['payment_confirmation']=='pending'){
									$current_status='pending';
								}else{
									$current_status='replied';
								}
								$this->Commoncountsfunctions->updateUserCounts($customer_id,'pao','applicant','ref_back',$current_status);*/
				///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
